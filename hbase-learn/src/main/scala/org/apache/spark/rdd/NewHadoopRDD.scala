...
import org.apache.hadoop.hbase.mapreduce.UDETableInputFormatBase
...
class NewHadoopRDD[K, V](
  ...
  override def compute(theSplit: Partition, context: TaskContext): InterruptibleIterator[(K, V)] = {
    val iter = new Iterator[(K, V)] {
      ...
      private var _reader : RecordReader[K, V] = _
//      context.addTaskFailureListener((context, e) => close())
      private var reader : RecordReader[K, V] =
        try {
          _reader = format.createRecordReader(
